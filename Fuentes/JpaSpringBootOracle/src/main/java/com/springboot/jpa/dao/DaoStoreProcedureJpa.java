...
public class DaoStoreProcedureJpa implements Serializable {
	 ...
	 */
	public List<Object[]> obtenerNotificaciones() {

		try {

			log.info("Procediendo a obtener el detalle de los clientes");
			StoredProcedureQuery storedProcedureQuery = entityManager .createStoredProcedureQuery(prop.getPROCEDURE_DEVUELVE_CURSOR());

			storedProcedureQuery.registerStoredProcedureParameter("P_ESTADO", Integer.class, ParameterMode.IN);
			storedProcedureQuery.registerStoredProcedureParameter("OUT_ESTADO", Class.class, ParameterMode.REF_CURSOR);
			storedProcedureQuery.setParameter("P_ESTADO", 1);

			storedProcedureQuery.execute();

			List transactionCursor = storedProcedureQuery.getResultList();
